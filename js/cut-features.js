...
window.updateOverlayProps = function(id){
  ...
  const posSection = ov.x!==undefined ? `
    ...
    <div class="prop-row"><span class="prop-label">Width</span>
      ${inp('op-w',fmtN(ov.w),0.01,`window._overlays.find(o=>o.id==='${ov.id}').w=Math.max(0.01,parseFloat(this.value));if(window.syncCutVid)syncCutVid();`)}
    </div>
    <div class="prop-row"><span class="prop-label">Height</span>
      ${inp('op-h',fmtN(ov.h),0.01,`window._overlays.find(o=>o.id==='${ov.id}').h=Math.max(0.01,parseFloat(this.value));if(window.syncCutVid)syncCutVid();`)}
    </div>
    ...
    [[0,0,'nw-resize'],[1,0,'ne-resize'],[0,1,'sw-resize'],[1,1,'se-resize']].forEach(([cx,cy,cur])=>{
      ...
      h.addEventListener('mousedown', e=>{
        ...
        const mv=(e2)=>{
          ...
          if(cy===0){ov.y=oy2+dy/2;ov.h=Math.max(0.04,oh-dy);}
          else{ov.y=oy2+dy/2;ov.h=Math.max(0.04,oh+dy);}
          redraw();
          if(window.syncCutVid) syncCutVid();
        };
        const up=()=>{
